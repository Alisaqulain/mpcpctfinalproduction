...
export async function POST(req) {
  try {
    ...
    const { questionsText, topicId } = body;

    if (!questionsText || !topicId) {
      return NextResponse.json(
        { error: "Missing questionsText or topicId" },
        { status: 400 }
      );
    }
    ...
    if (parsedQuestions.length === 0) {
      return NextResponse.json(
        { error: "No valid questions found. Please check the format. Make sure each question is on a single line with format: Question (Hindi?) A. Opt1 (Hindi1) B. Opt2 (Hindi2) C. Opt3 (Hindi3) D. Opt4 (Hindi4) Ans: X. Answer" },
        { status: 400 }
      );
    }
    ...
    const questionsToInsert = parsedQuestions.map((q, index) => {
      ...
      return {
        ...
        explanation_hi: q.explanation_hi || `सही उत्तर ${String.fromCharCode(65 + q.correctAnswer)} है। ${q.options_en[q.correctAnswer]}`,
        difficulty: 'medium',
