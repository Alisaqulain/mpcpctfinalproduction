    ...
    <div className="min-h-screen bg-white p-2">
      ...
      <div className="bg-white flex flex-col sm:flex-row items-center justify-between p-2 border-b">
        <div className="bg-white-50 border-gray-300 px-4 py-4 flex gap-2 text-xs flex-wrap justify-center sm:justify-start">
          <button className="text-[15px] text-[#000] px-2 py-3 border-1 border-gray-300 rounded">
            <span className="my-4">CPCT Actual</span>
          </button>
          <button className="border px-2 py-2 rounded text-[#000] border-gray-300">English Typing</button>
          <button className="border px-3 py-1 rounded border-gray-300 bg-[#290c52] text-[#fff] font-semibold">हिंदी टाइपिंग</button>
        </div>
        ...
        <div className="flex items-center space-x-4 mt-2 sm:mt-0">
          <button
            ...
            className={`flex items-center px-2 py-1 rounded text-sm font-semibold ${
              isSoundOn ? "bg-blue-100 text-green-700" : "bg-red-100 text-red-700"
            }`}
          >
      ...
      <div className="flex flex-col lg:flex-row">
        <div className="flex-1 bg-white p-4 order-2 lg:order-1">
          ...
          <div
            ref={containerRef}
            className="border p-2 mt-5 overflow-y-auto h-48 rounded border-gray-300 leading-relaxed text-[18px] md:text-[22px] space-y-1 pl-6"
          >
            {Array.from({ length: Math.ceil(words.length / 10) }).map((_, rowIndex) => {
              const rowWords = words.slice(rowIndex * 10, rowIndex * 10 + 17);
              return (
                <div key={rowIndex} className="border-b-2 border-gray-200 pb-2 mb-2">
                  {rowWords.map((word, i) => {
                    ...
                    return (
                      <span
                        ...
                        className={`mr-4 ${getWordClass(word, actualIndex)}`}
                      >
                        {word}
                      </span>
                    );
        ...
        <div className="w-full lg:w-60 bg-blue-50 p-4 border-l h-auto lg:h-[520px] order-1 lg:order-2">
          ...
          <div className="mt-4 grid grid-cols-2 gap-2 sm:gap-4 text-sm">
            {[{ label: "Correct Word", value: correctWords, color: "text-green-700" },
              ...
              >
                <div className="bg-black text-center text-white text-[10px] font-semibold py-[1px]">
                  {item.label}
                </div>
                <div className={`bg-white text-center ${item.color || "text-black"} text-[14px] font-bold`}>
                  {item.value}
                </div>
              </div>
            ))}
          ...
          <button className="bg-green-800 text-white w-full mt-4 lg:mt-6 p-2 rounded">
            <a href="/exam/hindi-result">Submit Section</a>
          </button>
        </div>
      </div>
    </div>
