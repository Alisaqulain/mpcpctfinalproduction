  ...
  return (
    <div className="p-4 flex flex-col gap-6 w-full min-h-full" style={{ minHeight: '100dvh' }}>
      ...
      <div className="flex-1 flex flex-col items-center gap-6 mobile-stack">
        ...
        <div 
          className="flex flex-nowrap justify-between items-center gap-1 relative mt-6 px-2 typing-prompt-container w-full portrait-typing-prompt touch-manipulation cursor-pointer active:opacity-90"
...
function LandscapeMobileView({
  ...
  return (
    <div className="pl-4 pr-4 pt-4 pb-4 flex flex-col md:flex-row gap-6 w-full min-h-full landscape-mobile-view-container" style={{ minHeight: '100dvh' }}>
      {/* Theme Toggle and Settings - Landscape: safe area, larger tap targets */}
      <div
        className="fixed top-0 right-0 z-50 flex items-center gap-3 p-3"
        style={{ paddingTop: 'max(12px, env(safe-area-inset-top, 12px))' }}
      >
        <button
          onClick={() => setShowSettings(!showSettings)}
...
function KeyboardApp() {
  ...
  // Mobile/WebView: process typed value and clear. Use lastProcessedLengthRef so we only process new chars.
  const processMobileInput = useCallback((value) => {
    ...
    lastProcessedValueRef.current = value.length;
    setMobileInputValue("");
  }, [updateHandImages, processKeyForPractice]);
  ...
  // Native input/change listeners so Expo WebView and some mobile browsers get typing (React onChange may not fire)
  useEffect(() => {
  ...
  return (
    ...
    >
      {/* Type with white keyboard: tap key row or this bar, then type. data-typing-input for app WebView. */}
      <input
        type="text"
        ref={inputRef}
        data-typing-input
        ...
        onFocus={(e) => { const el = e.target; lastProcessedValueRef.current = 0; el.value = ""; el.setSelectionRange(0, 0); }}
        className="keyboard-page-type-input absolute left-0 top-[30%] w-11 h-11 opacity-0 border-0 p-0 m-0 outline-none text-base"
        style={{ fontSize: 16 }}
