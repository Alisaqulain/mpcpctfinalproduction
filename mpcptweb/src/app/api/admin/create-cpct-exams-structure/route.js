  ...
  try {
    ...
    for (let examNum = 1; examNum <= 20; examNum++) {
      try {
        ...
        // Delete existing sections and parts for this exam (to recreate fresh)
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully created ${createdExams.length} CPCT exams with structure (no questions added)`,
      exams: createdExams,
      summary: {
      ...
      note: "Exams created with 3 sections: Section A (75 min, 5 parts - NO QUESTIONS), Section B (English Typing - 15 min, linked to skill lessons), Section C (Hindi Typing - 15 min, linked to skill lessons). Questions need to be added manually to Section A parts."
    });

  } catch (error) {
    console.error("Create CPCT exams structure error:", error);
    return NextResponse.json({ 
      error: error.message || "Failed to create CPCT exams structure" 
    }, { status: 500 });
  }
}
