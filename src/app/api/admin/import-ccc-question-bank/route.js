...
export async function POST(req) {
  try {
    ...
    const { questionsText } = body;

    if (!questionsText) {
      return NextResponse.json(
        { error: "Missing questionsText" },
        { status: 400 }
      );
    }
    ...
    // Get current question count to append new questions
    const existingCount = await Question.countDocuments({
      examId: "CCC_QUESTION_BANK"
    });

    // Insert questions into question bank (append, don't replace)
    const questionsToInsert = parsedQuestions.map((q, index) => {
      return {
        id: `ccc-bank-q-${existingCount + index + 1}`, // Continue numbering from existing
        examId: "CCC_QUESTION_BANK", // Special identifier for question bank
        sectionId: "CCC_QUESTION_BANK_SECTION", // Dummy sectionId for question bank
        partId: "CCC_QUESTION_BANK_PART", // Dummy partId for question bank
    ...
    return NextResponse.json({
      success: true,
      message: `Successfully imported ${questionsToInsert.length} questions into CCC question bank`,
