...
export default function ExamSection() {
  ...
  // Build tabs from exam types: id = key for non-topic-wise, "topicwise" for Topic Wise tab
  const tabs = examTypes.map((t) => ({
    id: t.isTopicWise ? "topicwise" : t.key,
    label: t.label,
    key: t.key,
    isTopicWise: t.isTopicWise,
  }));

  // Fetch exam types first, then exams for each type
  useEffect(() => {
    const fetchExamTypes = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setExamTypes(types);
          if (types.length > 0 && activeTab === null) {
            setActiveTab(types[0].isTopicWise ? "topicwise" : types[0].key);
          }
        }
      } catch (error) {
  ...
  useEffect(() => {
    ...
    const fetchExams = async () => {
      try {
        setLoading(true);
        const examData = {};
        const keysToFetch = examTypes.filter((t) => !t.isTopicWise).map((t) => t.key);
    ...
    <div className="min-h-screen bg-[#fff] flex flex-col">
      ...
      <div className="sticky top-0 z-50 bg-white shadow-sm">
        <div className="max-w-4xl mx-auto px-4 py-4">
          ...
          {/* Tab Navigation - from admin-configured exam types */}
          <div className="flex border border-gray-300 rounded-full overflow-hidden shadow-sm flex-wrap">
            {tabs.map((tab) => (
              <button
                key={tab.id}
                onClick={() => setActiveTab(tab.id)}
                className={`flex-1 min-w-0 py-4 text-[10px] md:text-[16px] font-medium transition-colors duration-300 border-l border-gray-300 first:border-l-0 ${
                  activeTab === tab.id
        ...
        <div className="max-w-4xl mx-auto px-4 py-6">
          ...
          {(examTypes.length === 0 || loading) && (
            <div className="text-center py-8">
              <p className="text-gray-600">{examTypes.length === 0 ? "Loading..." : "Loading exams..."}</p>
            </div>
          )}
                  ...
                  <div className="space-y-4">
                    {topics.map((topic) => (
                      ...
                      >
                        ...
                        <button 
                          onClick={() => router.push(`/exam/exam-login?topicId=${topic.topicId}&type=TOPICWISE`)}
